...
function parseRegex(regex) {
    ...
    // Parses a sequence of normal characters and returns the corresponding Regular expression node.
    function parseNormal() {
        // Initialize an empty string to accumulate the normal characters.
        let normalChars = "";

        // Keep parsing until the current character is not a normal character.
        while ([LEFT_PAREN, RIGHT_PAREN, STAR, PIPE, DOT].indexOf(currentChar) === -1) {
            normalChars += currentChar;
            index++;
            currentChar = regex[index];
        }

        // If there were no normal characters, return null.
        if (normalChars === "") {
            return null;
        }

        // Otherwise, return a Normal node with the accumulated normal characters.
...
console.log(result); // Str {regexpList: [Normal, Str {regexpList:
// [Or {left: Normal {char: "b"}, right: Normal {char: "c"}}, ZeroOrMore {regexp: Any}]}]}